...
Upstream INFO logs include transcript/reference content, so worker suppresses INFO and stdout; no raw audio recorded. Upstream warnings/errors may still contain diagnostics. EC2 disk, boot logs, and temporary artifact bucket remain subject to lifecycle cleanup.
...
- `GET /health`: `mode`, `interviewProtocol: 1`, `upstreamReady`, `busy`. Desktop rejects older generic gateway before architect work. Ready HTTP model server still needs per-session ASR initialization.
