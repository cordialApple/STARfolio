...
describe('technical cite-guarantee', () => {
  it('first question falls back to a real chunk when the model cites nothing valid', async () => {
    const provider = mockProvider({ question: 'q', cited_chunk_ids: ['not-a-real-chunk'] })
    const out = await firstTechnicalQuestion(CONFIG, CHUNKS, provider)
    expect(out.cited_chunk_ids).toEqual(['c1'])
  })

  it('a non-terminal follow-up always cites at least one real chunk', async () => {
    const provider = mockProvider({ feedback, next_kind: 'question', next_text: 'next?', cited_chunk_ids: [] })
    const turn = await evaluateTechnicalAnswer({ config: CONFIG, chunks: CHUNKS, asked: [], question: 'q', answer: 'my answer' }, provider)
  ...
  it('keeps only valid cited chunk ids and drops invented ones', async () => {
    const provider = mockProvider({ feedback, next_kind: 'question', next_text: 'next?', cited_chunk_ids: ['c2', 'ghost'] })
    const turn = await evaluateTechnicalAnswer({ config: CONFIG, chunks: CHUNKS, asked: [], question: 'q', answer: 'a' }, provider)
    expect(turn.cited_chunk_ids).toEqual(['c2'])
  })

  it('allows an empty citation list on a terminal (done) move', async () => {
    const provider = mockProvider({ feedback, next_kind: 'done', next_text: '', cited_chunk_ids: [] })
    const turn = await evaluateTechnicalAnswer({ config: CONFIG, chunks: CHUNKS, asked: ['a', 'b', 'c'], question: 'q', answer: 'a' }, provider)
