...
describe('HalfDuplexGate', () => {
  it('opens capture on a fresh default gate for any now', () => {
    ...
    expect(gate.isCaptureOpen(0)).toBe(true)
    expect(gate.isCaptureOpen(999999)).toBe(true)
  })

  it('closes capture while speaking regardless of now', () => {
    const gate = new HalfDuplexGate()
    gate.onTtsStart()
    expect(gate.isCaptureOpen(0)).toBe(false)
    expect(gate.isCaptureOpen(999999)).toBe(false)
  })
  ...
  it('holds capture shut through the guard tail then reopens', () => {
    ...
    gate.onTtsStart()
    gate.onTtsEnd(1000)
    expect(gate.isCaptureOpen(1249)).toBe(false)
    expect(gate.isCaptureOpen(1250)).toBe(true)
    expect(gate.isCaptureOpen(2000)).toBe(true)
  })
  ...
  it('respects a custom guardMs', () => {
    ...
    gate.onTtsStart()
    gate.onTtsEnd(1000)
    expect(gate.isCaptureOpen(1499)).toBe(false)
    expect(gate.isCaptureOpen(1500)).toBe(true)
  })
