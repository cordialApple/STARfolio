...
import { assertPublicHttpUrl, isIpInPrivateRange } from '../../src/main/ingest/fetch-url'
...
describe('isIpInPrivateRange', () => {
  it('flags private, loopback, link-local and cloud-metadata IPv4', () => {
    for (const ip of ['0.0.0.0', '10.1.2.3', '127.0.0.1', '169.254.169.254', '172.16.0.1', '172.31.255.1', '192.168.0.1'])
      expect(isIpInPrivateRange(ip), ip).toBe(true)
  })
  ...
  it('allows public addresses', () => {
    for (const ip of ['8.8.8.8', '1.1.1.1', '93.184.216.34', '2606:4700:4700::1111'])
      expect(isIpInPrivateRange(ip), ip).toBe(false)
  })
})
