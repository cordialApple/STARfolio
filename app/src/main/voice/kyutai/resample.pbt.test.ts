...
describe('LinearResampler (PBT)', () => {
  it('chunk-split invariance: split boundaries never change the resampled stream', () => {
    runProperty(
      defineSyntheticOrganicProperty(
        'resample/chunk-split-invariance',
        '1',
        'resampling output is invariant to input chunk boundaries'
      ),
      pcmChunks,
      (chunks) => {
  ...
  it('convex bound: every output sample lies within the input min/max', () => {
    runProperty(
      defineSyntheticOrganicProperty(
        'resample/convex-bound',
        '1',
        'resampled values stay within input bounds'
      ),
      pcmChunks,
      (chunks) => {
...
describe('BlockChunker (PBT)', () => {
  it('conservation: pushed samples reappear as a prefix, tail zero-padded on flush', () => {
    runProperty(
      defineSyntheticOrganicProperty(
        'chunker/sample-conservation',
        '1',
        'chunking preserves every sample and zero pads only the tail'
      ),
      pcmChunks,
      (chunks) => {
  ...
  it('every emitted block is exactly BLOCK_SAMPLES wide', () => {
    runProperty(
      defineSyntheticOrganicProperty(
        'chunker/block-width',
        '1',
        'every emitted audio block has the protocol width'
      ),
      pcmChunks,
      (chunks) => {
        const chunker = new BlockChunker(BLOCK_SAMPLES)
        for (const c of chunks)
          for (const b of chunker.push(c)) if (b.length !== BLOCK_SAMPLES) return false
        const tail = chunker.flush()
