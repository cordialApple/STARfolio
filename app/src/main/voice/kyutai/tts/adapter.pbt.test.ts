...
const errorOut = fc.record({
  type: fc.constant('Error' as const),
  message: fc.string({ maxLength: 12 })
})
...
describe('tts adapter (PBT)', () => {
  ...
  it('emits every Audio chunk in delivered order, samples conserved', () => {
    runProperty(
      defineSyntheticOrganicProperty(
        'tts/audio-order-preserved',
        '1',
        'TTS audio chunks preserve delivery order and samples'
      ),
      schedule,
      (msgs) => {
  ...
  it('onStart fires exactly once, before the first audio, only when audio flows', () => {
    runProperty(
      defineSyntheticOrganicProperty(
        'tts/start-fires-once-before-audio',
        '1',
        'TTS start fires once before the first audio chunk'
      ),
      schedule,
      (msgs) => {
  ...
  it('onEnd mirrors every Marker id in delivered order', () => {
    runProperty(
      defineSyntheticOrganicProperty(
        'tts/end-mirrors-markers',
        '1',
        'TTS end events preserve marker delivery order'
      ),
      schedule,
      (msgs) => {
  ...
  it('forwards every Error message in delivered order', () => {
    runProperty(
      defineSyntheticOrganicProperty(
        'tts/errors-forwarded',
        '1',
        'TTS errors preserve delivery order and text'
      ),
      schedule,
      (msgs) => {
  ...
  it('buffers pre-Ready calls and flushes them in call order', () => {
    ...
    runProperty(
      defineSyntheticOrganicProperty(
        'tts/pending-flushed-in-call-order',
        '1',
        'pre-ready TTS calls flush in call order'
      ),
      fc.array(call, { maxLength: 12 }),
      (calls) => {
        const transport = new FakeTransport()
        ...
        return transport.sent.every(
          (b, i) => JSON.stringify(decode(b)) === JSON.stringify(expected[i])
        )
      }
    )
  })
