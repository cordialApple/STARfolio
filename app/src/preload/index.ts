...
const api: IpcApi = {
  ...
  voice: {
    ...
    onPartial: (cb) => {
      const handler = (_: Electron.IpcRendererEvent, event: TranscriptEvent): void => cb(event)
  ...
  ai: {
    ...
    providerHasKey: (provider) => ipcRenderer.invoke('ai:provider:hasKey', { provider }),
    providerSetKey: (provider, key) => ipcRenderer.invoke('ai:provider:setKey', { provider, key }),
    providerDeleteKey: (provider) => ipcRenderer.invoke('ai:provider:deleteKey', { provider }),
