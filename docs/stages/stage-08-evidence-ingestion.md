...
- [x] 8.1 xlsx/csv: sheet flattening + numbers summary (Haiku) → draft with metrics prefilled → gap questions (Situation? Result? impact?) mandatory for evidence kind.
- [x] 8.2 Code files/folders/archives: walk with `.gitignore`/size/binary filters → repomix-style packing → draft from structure, languages, README claims.
- [x] 8.3 GitHub repo URL: REST tarball (optional PAT in safeStorage for private repos) → same packing path.
- [x] 8.4 Evidence-specific confirm UI: "here's what I can prove — tell me the story around it."
- [x] 8.5 Knowledge-graph layer: `entities`/`edges` migration; LLM entity extraction (people, teams, projects, tools) folded into the ingest confirm flow (extracted entities shown for approval alongside the STAR draft); backfill pass over existing experiences; Bank detail view gains a "connected to" panel (1–2-hop traversal: shared entities, shared skills).
