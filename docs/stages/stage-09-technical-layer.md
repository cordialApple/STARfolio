...
- [ ] 9.1 Corpus ingestion: reuse extractors → `corpus_docs`/`corpus_chunks` + chunk embeddings + FTS; corpus manager UI (add/remove, per-discipline).
- [ ] 9.2 Technical session mode: retrieve from corpus by discipline → generate a system-design/technical question → walk-through loop with discerning follow-ups probing stated design decisions; retrieved corpus-chunk IDs logged per question/follow-up (same provenance discipline as mode B).
- [ ] 9.3 Technical feedback rubric distinct from behavioral.
