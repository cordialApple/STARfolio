...
- [x] 2.1 Full schema migration (entities + joins + FTS **with sync triggers**; vec table can wait for Stage 3's queue).
- [x] 2.2 Guided STAR form: STAR fields, context, date range, skills/tags (autocomplete-with-create), metrics; save as draft or confirmed; edit/delete.
- [x] 2.3 Bank view: list + filters (skill, tag, context, date, status) + FTS5 keyword search; experience detail view; drafts surfaced for finishing.
- [x] 2.4 Repository unit tests; e2e: create → filter → edit → confirm.
