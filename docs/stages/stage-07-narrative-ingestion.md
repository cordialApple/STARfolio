...
- [ ] 7.1 Ingest framework: Extractor interface generalizing Stage 3's source persistence; imported files copied into the content-hash-named attachments dir (provenance survives moved/deleted originals); duplicate hash → offer to reuse the existing source; import wizard UI (drop zone + URL field); per-file review queue reusing the Stage-3 confirm flow.
- [ ] 7.2 txt/md + docx (`mammoth`) + pdf (`pdfjs-dist`; graceful "this looks scanned" failure).
- [ ] 7.3 Resume mode: multi-experience splitting — one document proposes N drafts, each reviewed separately.
- [ ] 7.4 URL ingestion: fetch → readability → markdown → propose.
- [ ] 7.5 Extractor tests against a real-file fixture corpus (incl. a real resume).
