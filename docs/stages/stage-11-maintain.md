...
- [ ] 11.1 Nudges: staleness banner ("last logged 3 weeks ago"), configurable OS-notification reminder, post-practice "bank that story?" prompts, and an optional launch-at-startup/tray-resident toggle so reminders reach the logger who stops opening the app.
- [ ] 11.2 JSON export/import + one-click DB backup from Settings.
- [ ] 11.3 electron-updater + GitHub Releases; final packaging pass (icon, installer, SmartScreen note in README).
- [ ] 11.4 Cost dashboard polish (spend by feature from `usage_log`); empty-state/onboarding pass (first-run: key setup → first brain dump).
